// serach the recoreds based on product category && and or condition
...
expressObj.put('/api/products/:id', function (req, resp) {
    ...
    var item = products.findIndex(products => products.PrdId == id)
...
// the put endpoint
expressObj.post('/api/search', function (req, resp) {
    var data = req.body.CatName
    // console.log('111111111111111',data)

    var filteredProducts=[];
    if (req.body.CatName) {
       var filteredProduct = products.filter(function (p, i) {
            return p.CatName == req.body.CatName;
        });
        filteredProducts.push(filteredProduct)
    }
    if (req.body.PrdId) {
        var filteredProduct = products.filter(function (p, i) {
            return p.PrdId == req.body.PrdId;
        });
        filteredProducts.push(filteredProduct)
    }
    ...
    if (req.body.PrdName) {
        var filteredProduct = products.filter(function (p, i) {
            return p.PrdName == req.body.PrdName;
        });
        filteredProducts.push(filteredProduct)
    }
    console.log('111111111111111', JSON.stringify(filteredProducts))
    resp.send({ status: 200, data: filteredProducts }); // response in JSON
});
